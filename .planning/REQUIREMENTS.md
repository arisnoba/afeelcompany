...
- [x] **INST-04**: 관리자가 이미지+캡션으로 인스타그램에 게시할 수 있다 (Development Mode)
- [x] **INST-05**: 게시 성공/실패 상태가 DB에 기록되고 관리자에게 표시된다
...
- [x] **AUTH-01**: 관리자가 환경변수 기반 비밀번호로 로그인할 수 있다
- [x] **AUTH-02**: 로그인 시 쿠키/세션이 발급되어 `/admin/*` 접근이 유지된다
- [x] **AUTH-03**: 미인증 사용자가 `/admin/*` 접근 시 로그인 페이지로 리디렉트된다
- [x] **AUTH-04**: 관리자가 로그아웃할 수 있다

### 이미지 업로드 (UPLD)

- [x] **UPLD-01**: 관리자가 드래그&드롭 또는 파일 선택으로 이미지를 업로드할 수 있다
- [x] **UPLD-02**: 업로드 시 메타데이터(제목, 브랜드명, 셀럽명, 카테고리)를 입력할 수 있다
- [x] **UPLD-03**: 업로드 용도를 선택할 수 있다 (웹 갤러리 / 인스타 / 둘 다)
- [x] **UPLD-04**: 클라이언트 사이드에서 이미지가 리사이즈된다 (max 2000px, 품질 80%)
- [x] **UPLD-05**: 업로드 진행률이 표시되고 성공/실패 피드백이 제공된다
- [x] **UPLD-06**: 이미지가 Vercel Blob에 저장되고 메타데이터가 DB에 기록된다

### 포트폴리오 관리 (PORT)

- [x] **PORT-01**: 관리자가 포트폴리오 아이템 목록을 테이블 뷰(썸네일, 제목, 브랜드, 상태)로 볼 수 있다
- [x] **PORT-02**: 관리자가 개별 아이템을 인라인 편집 또는 모달로 수정할 수 있다
- [x] **PORT-03**: 관리자가 웹/PDF 노출 여부를 토글할 수 있다
- [x] **PORT-04**: 관리자가 아이템 순서를 변경할 수 있다 (sort_order)
- [x] **PORT-05**: 관리자가 아이템을 삭제할 수 있다 (Blob + DB 동시 삭제)

### 회사 프로필 (PROF)

- [x] **PROF-01**: 관리자가 회사소개 문구를 편집할 수 있다
- [x] **PROF-02**: 관리자가 연락처 정보(이메일, 전화, 주소)를 편집할 수 있다
- [x] **PROF-03**: 관리자가 클라이언트 브랜드 로고를 추가/삭제/순서 변경할 수 있다
...
| INST-04~05 | Phase 3 | Complete |
| PDF-01~06 | Phase 2 | Complete |
| AUTH-01~04 | Phase 3 | Complete |
| UPLD-01~06 | Phase 3 | Complete |
| PORT-01~05 | Phase 3 | Complete |
| PROF-01~03 | Phase 3 | Complete |
...
*Last updated: 2026-03-27 after Phase 3 approval*
